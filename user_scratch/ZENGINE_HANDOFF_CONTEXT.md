...
**Packaged:** February 27, 2026 | **Phase:** 0 (exiting) | **Next:** Phase 1
...
| File                  | Status                                       |
| --------------------- | -------------------------------------------- |
| `engine/combat.py`    | Canonical stub — complete, Phase 0           |
| `CONTEXT.md`          | Single source of truth — committed           |
| `AGENT_ONBOARDING.md` | Opening/closing ritual templates — committed |

### Files that must be created immediately (pre-Phase 1)

| File                  | Purpose                                                      |
| --------------------- | ------------------------------------------------------------ |
| `DESIGN_VARIABLES.md` | All design variables with defaults — ready to populate       |
| `FUTURE.md`           | Post-MVP deferred systems — items already queued             |
| `DO_NOT_TOUCH.md`     | Locked files per phase — empty until Phase 1 locks something |

### Files created during Phase 1 (no code — contracts only)
...
All three Phase 1 blockers are now **resolved**. Phase 1 is unblocked.

Remaining TBD items (Phase 2 blockers — do not resolve yet):
